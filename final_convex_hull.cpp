...
void GUI_VISUALIZATION (std::stack<std::tuple<int,int, double>> graham_scan_stack, std::vector<std::tuple<int,int, double>> plotted_points){
// create the window that will open when the program is launched
    ...
    int numPoints = graham_scan_stack.size();
    ...
    while(!graham_scan_stack.empty())
    {
        ...
        convexHull.setPoint(i, sf::Vector2f(std::get<0>(graham_scan_stack.top())+100, std::get<1>(graham_scan_stack.top())+100));
    }
...
//--------------------------------------------------------------------------------
    ...
    while (window.isOpen())
    {
        ...
        int sizeOfPlottedPoints = plotted_points.size();
        for (int i = 0; i < sizeOfPlottedPoints; i ++) {
            sf::CircleShape point;
            point.setRadius(10);
            point.setFillColor(sf::Color(255,255,255));
...
int main (int argc, char**argv){
    ...
    //Now we push the lowest point back in so we can plot it in SFML
    plotted_points.push_back(lowestPair);

    GUI_VISUALIZATION(graham_scan_stack, plotted_points);
